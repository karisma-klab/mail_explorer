...
import sys
import subprocess
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QPushButton, QLineEdit, QTableWidget, QTableWidgetItem, QTextEdit, QSplitter, QDialog, QLabel, QPlainTextDocumentLayout
...
class SearchApp(QMainWindow):
    ...
    def run_search_command(self):
        ...
        command = f'find summarized/ -type f -print0 | pv -0 -n -s 7151 | xargs -0 -P 1 grep -R -i -m 1 "{spanish_friendly_str}" > test_search.txt'
    ...
    def populate_search_history(self):
        ...
        search_files = [f for f in os.listdir("searches/") if f.endswith(".txt")]
        ...
        for i, file_name in enumerate(search_files):
            ...
            results = sum(1 for _ in open(f"searches/{file_name}", encoding="latin-1"))
            create_date = datetime.datetime.fromtimestamp(os.path.getctime(f"searches/{file_name}")).strftime('%c')
    ...
    def show_search_result(self, row, _):
        ...
        file_path = f"searches/{file_name}"
